...
public static class HttpRequestExtentions
{
    ...
    {
        //return $"{request.Scheme}://localhost{(request.Host.Port == null ? "" : $":{request.Host.Port}")}{(needPathBase ? request.PathBase.ToString() : "")}{path}";
        return $"http://localhost{(request.Host.Port == null ? "" : $":{request.Host.Port}")}{(needPathBase ? request.PathBase.ToString() : "")}{path}";
    }

...
const MyMenu = () => {
    ...
    const mainteMenuList = [
        ...
        { linkLabelText: "ユーザー", spaUrlBase: constant.view_base, toUrl: `${constant.mainte_view_base}/user` },
        { linkLabelText: "事業コード", spaUrlBase: constant.view_base, toUrl: `${constant.mainte_view_base}/businessCode` },
    ]
...
const MenuGroup = ({ id, buttonText, linkInfoList, ...otherProps }) => {
    ...
    return (
        <>
            <Button aria-controls={id} color="inherit" aria-haspopup="true" onClick={handleButtonClick} className={classes.menu_button}>
                {buttonText}
            </Button>
            <Menu
                id={id}
    ...
    return (
        isSpa ?
            ...
            (
                ...
                <MenuItem component={Link} href={fullToUrl} underline="none" className={classes.menu_link} onClick={onClose}>{linkLabelText}</MenuItem>
            )
    )
}
